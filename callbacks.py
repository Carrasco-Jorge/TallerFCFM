...
class PlotBatch(Callback):
  ...
  def on_epoch_end(self, epoch, epoch_logs):
    # Plot examples
    img, msk = next(iter(val.data))
    pred = self.model.predict(img,batch_size=self.batch_size)

    fig, ax = plt.subplots(nrows=3, ncols=self.num_samples, figsize)
    for i in range(self.num_samples):
	ax[0][i].imshow(img[counter], vmin=0.0,vmax=1.0)
	ax[0][i].axis("off")

      ax[1][i].imshow(msk[counter], cmap='gray',vmin=0.0,vmax=1.0)
	ax[1][i].axis("off")

      ax[2][i].imshow(pred[counter], cmap='gray',vmin=0.0,vmax=1.0)
	ax[2][i].axis("off")
	...
	tfImage(f'Val. images epoch {epoch}', plot, step=0)
